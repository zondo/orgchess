...
Now we need a function to print the solution:

#+name: print-solutions
#+begin_src python :results silent
  def print_solutions(fen):
      for move, replies in mate_in_2(fen):
          print(f"1. {move}")

          for mating_move, replies in group_replies(replies).items():
              allreplies = ", ".join(replies)
              print(f"1. ... {allreplies}  2. {mating_move}")
#+end_src
...
Here's the complete mate-in-2 program:
